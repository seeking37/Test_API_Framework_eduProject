...
def get_testcase_yaml(file):
    ...
    try:
        with open(file, 'r', encoding='utf-8') as f:
            ...
            # len(data) = 1:说明是一个单接口的测试用例文件，如addUser.yaml，只有一个- baseInfo（要将 baseInfo 和每个 testCase 组合）
            # len(data) > 1: 多接口文件，如BusinessScenario.yml，通常用于业务流程测试，每个步骤往往只需要一个case_name。直接返回即可，无需组合
            if len(data) <= 1:
                yam_data = data[0]
...
class ReadYamlData:
    ...
    def write_yaml_data(self, value):
        ...
        file = None
        file_path = FILE_PATH['EXTRACT'] # 'C:\\tocode\\Project\\Pyproject\\row_Test-Automation-Framework-main\\extract.yaml'
...
if __name__ == '__main__':
    # 获取所有测试用例
    testcase_list = get_testcase_yaml("../testcase/Single interface/addUser.yaml")
    # 遍历每个测试用例
    for base_info, testcase in testcase_list:
        print("Base Info:", base_info)
        print("Test Case:", testcase)
        print("---")

    testcase_list2 = get_testcase_yaml("../testcase/Business Scenario/BusinessScenario.yml")
    for case_info in testcase_list2:
