...
class ConnectMysql:
    ...
    def __init__(self):
        try:
            self.conn = self.get_pool().connection()
            self.cursor = self.conn.cursor(cursor=pymysql.cursors.DictCursor)
            logs.info("成功从连接池获取MySQL连接")
        except Exception as e:
            logs.error(f"获取数据库连接异常: {e}")
            raise
...
class ConnectSSH(object):
    """连接SSH终端服务"""
    # 这个类主要用于测试框架与远程服务器的交互，特别是在需要查看服务器日志或执行远程命令时使用。它不是数据库连接，而是一个远程服务器管理工具。
    def __init__(self,
                 host=None,
                 port=22,
                 username=None,
